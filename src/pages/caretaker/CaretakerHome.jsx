...
export default function CaretakerHome() {
    ...
    useEffect(() => {
        ...
        const q = query(collection(db, "users"), where("caretakerUid", "==", currentUser.uid));
    ...
    const handleLinkPatient = async (e) => {
        ...
        try {
            ...
            const q = query(collection(db, "users"), where("patientId", "==", linkId));
    ...
    return (
        <div className="fade-in">
            ...
            <div className="card mb-8 bg-blue-50 border-blue-200">
                ...
                <form onSubmit={handleLinkPatient} className="flex gap-4 items-center">
                    <input
                        ...
                        required
                    />
                    <button type="submit" className="btn btn-primary">Link & Monitor</button>
                </form>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {!loading && patients.length === 0 && <p className="text-muted italic col-span-full">No patients linked yet.</p>}
            </div>
        </div>
    );
}
...
function PatientStatusCard({ patient }) {
    ...
    // Style logic
    const statusColor = isRisk ? 'text-red-600' : isAttention ? 'text-yellow-600' : 'text-green-600';
    const statusBg = isRisk ? 'bg-red-50' : isAttention ? 'bg-yellow-50' : 'bg-green-50';
    const border = isRisk ? 'border-red-200' : isAttention ? 'border-yellow-200' : 'border-gray-200';

    return (
        <motion.div whileHover={{ y: -5 }} className={`card border-l-4 ${isRisk ? 'border-l-red-500' : 'border-l-green-500'}`}>
            <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center ${statusBg} ${statusColor}`}>
                        <User size={24} />
                    </div>
                    <div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="text-center p-2 bg-gray-50 rounded">
                    ...
                    <p className={`font-bold ${statusColor} text-lg`}>{patient.adherenceScore || 100}%</p>
                </div>
                <div className="text-center p-2 bg-gray-50 rounded">
                    <p className="text-xs text-muted">Streak</p>
                    <p className="font-bold text-gray-800 text-lg">{patient.streak || 0}</p>
                </div>
            </div>
            ...
            <Link to={`/dashboard/caretaker/monitor/${patient.id}`} className="btn btn-outline w-full flex justify-center items-center gap-2">
